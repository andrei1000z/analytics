...
export type DashboardData = {
  ...
  uniqueDevicesDelta: number | null;
  topReferrer: string;
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const topReferrer =
    [...refCounts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? "Direct";
  ...
  const uniqueDevicesCurr = new Set(
    last7.map((r) => `${r.device_type ?? "?"}|${r.url}`)
  ).size;
  const uniqueDevicesPrev = new Set(
    prev7.map((r) => `${r.device_type ?? "?"}|${r.url}`)
  ).size;
  ...
  return {
    ...
    uniqueDevices: uniqueDevicesCurr,
    uniqueDevicesDelta: pct(uniqueDevicesCurr, uniqueDevicesPrev),
    topReferrer,
    conversionRate: "—",
    conversionDelta: null,
    traffic,
